...
public class CognitiveReader
{
        ...
        int topN = 3)
    {
        ...
        for (int i = 0; i < featureResults.Count; i++)
        {
            ...
            // ★ 反馈状态过滤 + 权重修正
            if (!string.IsNullOrEmpty(record.FeedbackStatus))
            {
                switch (record.FeedbackStatus)
                {
                    case "rejected":
                        Console.WriteLine($"🧠 卡片 {record.Id} 因反馈 rejected 被丢弃");
                        continue;
                    case "confirmed":
                        Console.WriteLine($"🧠 卡片 {record.Id} 反馈 confirmed，加分优先");
                        finalConfidence *= 1.15;
                        break;
                    case "ignored":
                        Console.WriteLine($"🧠 卡片 {record.Id} 反馈 ignored，中性处理（不升不降）");
                        // 中性，不做权重修正
                        break;
                    case "partial":
                        // 中性，不做调整
